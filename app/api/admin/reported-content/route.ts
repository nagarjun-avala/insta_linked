...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const formattedReportedContent = reportedPosts.map(report => {
    ...
    const aggregatedReports = formattedReportedContent.reduce<FormattedReport[]>((acc, current) => {
      ...
      if (existingReport) {
        ...
        if (new Date(current.lastReportDate) > new Date(existingReport.lastReportDate)) {
